...
* El sistema debe permitir a los usuarios visualizar imágenes patrimoniales de forma individual y agrupadas en conjuntos o clusters, según criterios de los curadores. Además, para la visualización del contenido no es necesario estar registrado dentro del sistema.
* El sistema debe permitir la creación y visualización de líneas de tiempo que representen el recorrido y la producción visual de Robert Gerstmann.
* El sistema debe ofrecer una vista de mapa interactiva que permita la exploración geográfica del contenido patrimonial asociado a las imágenes.

[[funcionalidades-de-busqueda-y-filtrado-rf-02]]
=== 3.2. Funcionalidades de Búsqueda y Filtrado (RF-02)

* El sistema debe permitir a los usuarios buscar y filtrar imágenes patrimoniales utilizando criterios como palabras clave, año y ubicación geográfica.
* El sistema debe integrar herramientas de búsqueda avanzada que faciliten la localización eficiente de contenido dentro de la plataforma.
* El sistema debe ofrecer ejemplos de búsqueda (prompts de ejemplo) y funcionalidades de búsqueda interactiva que orienten a los usuarios en la exploración del contenido disponible.

[[generacion-de-narrativas-rf-03]]
=== 3.3. Generación de Narrativas (RF-03)

* El sistema permite el uso de inteligencia artificial para generar interpretaciones y narrativas a partir de imágenes patrimoniales
* Los usuarios comunes pueden interactuar con la IA de forma exploratoria, sin que estas respuestas se almacenen ni modifiquen el conocimiento del sistema.
* Las narrativas oficiales que se publican en la plataforma son generadas con apoyo de IA y revisadas, editadas y validadas por curadores o expertos, manteniendo trazabilidad y fuentes visibles.

[[gestion-de-contenido-rf-04]]
=== 3.4. Gestión de Contenido (RF-04)

* El sistema debe permitir a los Curadores y Administradores agregar, editar y validar los metadatos asociados a las imágenes patrimoniales.
* El sistema debe permitir a los Curadores y Administradores revisar, modificar y aprobar las descripciones y contenidos generados por la inteligencia artificial antes de su publicación oficial.
* El sistema debe mantener registro de las modificaciones realizadas sobre los metadatos y descripciones, indicando el usuario responsable y la fecha de cada cambio.

[[descarga-de-archivos-rf-05]]
=== 3.5. Descarga de Archivos (RF-05)

* El sistema debe permitir a los usuarios descargar las imágenes patrimoniales disponibles en la plataforma, de acuerdo con los permisos y restricciones definidos.
* El sistema debe permitir la descarga de los textos y narrativas generadas o validadas dentro del sistema, respetando los niveles de acceso correspondientes.


[[Ubicación-por-lugares-y-zonas-rf-06]]
=== 3.6. Ubicación por lugares y zonas (RF-06)

* El sistema debe permitir la visualización de imágenes patrimoniales sobre un mapa interactivo, con opciones de filtrado por zona geográfica y país.
* El sistema debe permitir asociar las imágenes a lugares emblemáticos (gazetteer).
* El sistema debe ofrecer funcionalidades de búsqueda y auto sugerencia de lugares geográficos.
* El sistema debe soportar la asociación de múltiples ubicaciones geográficas a una misma imagen.


[[Etiquetado-temático-rf-07]]
=== 3.7. Etiquetado temático (RF-07)

* El sistema permite evaluar imágenes mediante inteligencia artificial para sugerir etiquetas temáticas basadas en su contenido visual y textual.
* Las etiquetas sugeridas pueden ser revisadas y editadas por curadores o expertos antes de su incorporación al sistema.
* Los usuarios comunes pueden visualizar estas sugerencias, pero no pueden incorporarlas directamente a la metadata oficial.


[[Líneas-de-tiempo-paralelas-rf-08]]
=== 3.8. Líneas de tiempo paralelas (RF-08)

* El sistema debe permitir la visualización de múltiples líneas de tiempo en paralelo.
* Cada línea de tiempo debe representar distintos ejes temporales, tales como hitos biográficos, acontecimientos históricos mundiales y eventos relevantes del contexto latinoamericano.
* El sistema debe permitir la navegación y comparación entre estos ejes temporales de forma sincronizada.
...
* El sistema debe permitir la creación de grupos de trabajo o proyectos para la colaboración en procesos de investigación.
* El sistema debe permitir asignar permisos diferenciados a los miembros de cada grupo, de acuerdo con sus roles y responsabilidades.
* El sistema debe permitir compartir información, contenidos y avances de investigación únicamente entre los miembros autorizados de cada grupo.


[[Análisis-IA-con-trazabilidad-rf-11]]
=== 3.11. Análisis IA con trazabilidad  (RF-11)

* La IA realiza análisis, pero cada salida se etiqueta como Veraz (con fuente primaria comprobable) o Verosímil (interpretación); siempre muestra fuentes.
...
* El sistema debe ofrecer opciones de filtrado de colecciones por criterios como época, lugar y temática.
* El sistema debe permitir ordenar las colecciones según distintos criterios, incluyendo “más vistas” y “más recientes”.
* El sistema debe permitir acceder a una vista dedicada de cada colección para su exploración detallada.
