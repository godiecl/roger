"""
Use case: extract Attribute 03 (Geographic Reference) from a photograph file.

Flow mirrors ExtractChronologyUseCase with the geographic analyzer factory.
"""
...
from app.infrastructure.analysis.analyzer_factory import create_geographic_analyzer
from app.shared.domain.exceptions import EntityNotFoundError


class ExtractGeographicUseCase:

    def __init__(self, repository: ITaxonomyRepository, session: AsyncSession):
        self.repository = repository
        self.session = session

    async def execute(self, photograph_id: int, triggered_by: int) -> GeographicReference:
        # 1. Verify photograph
        ...
        job = AnalysisJobModel(
            ...
            attribute_type=AnalysisAttributeType.GEOGRAPHIC,
        ...
        if analysis.get("error"):
            ...
            await self.session.flush()
            raise RuntimeError(
                f"Error al analizar georeferencia para fotografía {photograph_id}: "
                f"{analysis['error']}"
            )

        # 6. Supersede existing ACTIVE records
        await self.repository.supersede_active_geographic(photograph_id)

        # 7. Save new record
        record = GeographicReference(
            photograph_id=photograph_id,
            status=AttributeStatus.ACTIVE,
            source_type=SourceType.AI,
            location_type=analysis.get("location_type"),
            geographic_location=analysis.get("geographic_location"),
            latitude=analysis.get("latitude"),
            longitude=analysis.get("longitude"),
            location_radius_km=analysis.get("location_radius_km"),
            signage_found=analysis.get("signage_found"),
            architectural_landmarks=analysis.get("architectural_landmarks"),
            landscape_features=analysis.get("landscape_features"),
        ...
        saved = await self.repository.save_geographic(record)
