                  ...
                  <td align="center">
                    <div style="background:#f4f4f5;border-radius:12px;padding:24px 16px;display:inline-block;width:100%;">
                      ...
                      <p style="margin:16px 0 0;font-size:11px;color:#999;">
                        ¿Tienes problemas para copiar? Usa este código:
                      </p>
                      <p style="margin:6px 0 0;font-size:22px;font-weight:800;font-family:monospace;letter-spacing:6px;color:#1d1d1f;user-select:all;">
                        {code}
                      </p>
                    </div>
                  </td>
...
<body style="margin:0;padding:0;background:#f4f4f5;font-family:system-ui,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0">
    ...
    def send_email_change_confirmation(self, to: str, confirm_url: str) -> None:
        """Send email change confirmation link."""
        subject = "Confirma tu nuevo correo electrónico — Archivo ROGER"
        html = f"""
<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8" /></head>
<body style="margin:0;padding:0;background:#f4f4f5;font-family:system-ui,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center" style="padding:40px 16px;">
        <table width="100%" style="max-width:520px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              <h1 style="margin:0 0 12px;font-size:20px;font-weight:700;color:#1d1d1f;">Confirma tu nuevo correo</h1>
              <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.6;">
                Se ha solicitado cambiar el correo electrónico asociado a tu cuenta ROGER. Haz clic en el botón para confirmar el cambio.
              </p>
              <p style="margin:0 0 32px;font-size:13px;color:#999;">
                Este enlace es válido por <strong style="color:#555;">30 minutos</strong>. Si no solicitaste este cambio, ignora este correo.
              </p>
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td align="center">
    ...
    def send_welcome_email(self, to: str, full_name: str | None, username: str, password: str) -> None:
        """Send welcome email with credentials for admin-created accounts."""
        name_display = full_name or username
        subject = "Bienvenido/a al Archivo ROGER"
        html = f"""
<!DOCTYPE html>
<html lang="es">
<head><meta charset="utf-8" /></head>
    ...
    <tr>
      <td align="center" style="padding:40px 16px;">
        <table width="100%" style="max-width:520px;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
          ...
          <tr>
            <td style="padding:36px 32px 28px;">
              <h1 style="margin:0 0 12px;font-size:20px;font-weight:700;color:#1d1d1f;">Bienvenido/a, {name_display}</h1>
              <p style="margin:0 0 24px;font-size:15px;color:#555;line-height:1.6;">
                Se ha creado una cuenta para ti en el Archivo ROGER. A continuación encontrarás tus credenciales de acceso:
              </p>
              <div style="background:#f4f4f5;border-radius:10px;padding:20px 24px;margin-bottom:24px;">
                <p style="margin:0 0 8px;font-size:13px;color:#999;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Correo electrónico</p>
                <p style="margin:0 0 16px;font-size:15px;color:#1d1d1f;font-family:monospace;">{to}</p>
                <p style="margin:0 0 8px;font-size:13px;color:#999;font-weight:600;text-transform:uppercase;letter-spacing:1px;">Contraseña temporal</p>
                <p style="margin:0;font-size:18px;color:#1d1d1f;font-family:monospace;font-weight:700;letter-spacing:2px;">{password}</p>
              </div>
              <div style="background:#fff8ed;border:1px solid #fcd34d;border-radius:8px;padding:12px 16px;">
                <p style="margin:0;font-size:12px;color:#92400e;line-height:1.5;">
                  Por seguridad, te recomendamos cambiar tu contraseña desde tu perfil la primera vez que ingreses.
                </p>
              </div>
            </td>
          </tr>
